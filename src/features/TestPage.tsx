...
import { LoginInterface } from "../app/model/LoginInterface";
import agent from "../app/api/agent";
import { store } from "../app/store/store";
import { Formik } from "formik";

import { useNavigate } from "react-router-dom";
import { RegistrationDataSubmissionValue } from "../app/model/RegistrationDataSubmission";

function TestPage() {
  const defaultUserLoginValue: LoginInterface = {
    username: "",
    email: "",
    password: "",
  };

  const [submissionValue, setSubmissionValue] =
    useState<RegistrationDataSubmissionValue>(
      new RegistrationDataSubmissionValue()
    );
  const [userLoginValue, setUserLoginValue] = useState(defaultUserLoginValue);
  ...
  useEffect(() => {
    ...
    const DisplayName_: string = params.get("DisplayName") || "";
    if (DisplayName_ !== "") {
      const token_: string = params.get("Token") || "";
      store.CommonStore.setToken(token_);
      console.log(token_);
    }
  }, []);

  const onRegistrationFormChange = <
    P extends keyof RegistrationDataSubmissionValue
  >(
    prop: P,
    value: RegistrationDataSubmissionValue[P]
  ) => {
    setSubmissionValue({ ...submissionValue, [prop]: value });
  };

  const onLoginDetailChange = <P extends keyof LoginInterface>(
    prop: P,
    value: LoginInterface[P]
  ) => {
    setUserLoginValue({ ...userLoginValue, [prop]: value });
  };

  async function handleLogin() {
    try {
      const user = await agent.Account.login(userLoginValue);
      if (user.token === "NoUser") {
        console.warn("Login Failed.");
        return false;
      }

      sessionStorage.setItem("user", JSON.stringify(user)); // Prevent HMR issues
      store.CommonStore.setToken(user.token);
      navigate("/main"); // Ensure soft navigation
    } catch (error) {
      throw error;
    }
  }
  ...
  async function handleAccountRegistration() {
    try {
      const result: any = await agent.Account.submitRegistration(
        submissionValue
      );
      if (result.message === "Success") {
        document.getElementById("CreateAccountHref")?.click();
      } else {
  ...
  return (
    <>
      ...
      <main>
        <section className="section-about" id="section-about">
          ...
          <div className="row">
            <div className="col-1-of-2">
              ...
              <p className="paragraph">
                IoT soal secara rutin memungkinkan siswa untuk mengenali pola
                soal, mempelajari strategi pengerjaan yang efektif, dan
                mengurangi kecemasan menghadapi ujian sesungguhnya
              </p>
              <h3 className="heading-tertiary u-margin-bottom-small">
              ...
              <div className="book__form">
                ...
                <div className="form__group">
                  <input
                    ...
                    onChange={(e) => {
                      onLoginDetailChange("email", e.target.value);
                    }}
                    required
                  />
                ...
                <div className="form__group">
                  <input
                    ...
                    required
                    onChange={(e) => {
                      onLoginDetailChange("password", e.target.value);
                    }}
                  />
        ...
        >
          <div className="u-center-text u-margin-bottom-medium">
            <h2 className="heading-secondary">IoT Dimana Saja Dengan Gratis</h2>
          </div>
                    ...
                    <>
                      <div className="form__group">
                        <input
                          ...
                          id="username"
                          value={submissionValue.username}
                          onChange={(e) => {
                      ...
                      <div className="form__group">
                        <input
                          ...
                          placeholder="Email"
                          id="nama"
                          value={submissionValue.email}
                          onChange={(e) => {
                            onRegistrationFormChange("email", e.target.value);
                          }}
                          required
                        />
                        <label htmlFor="email" className="form__label">
                      ...
                      <div className="form__group">
                        <input
                          ...
                          placeholder="Password"
                          id="password"
                          value={submissionValue.password}
                          required
                          onChange={(e) => {
                            onRegistrationFormChange(
                              "password",
                      ...
                      <div className="form__group">
                        <input
                          ...
                          id="appliedInstance"
                          value={submissionValue.appliedInstance}
                          onChange={(e) => {
                            onRegistrationFormChange(
                              "appliedInstance",
                              e.target.value
